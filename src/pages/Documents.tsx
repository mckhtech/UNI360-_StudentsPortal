...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      console.log("[Documents] 🗺️ Filename to ID map:", Array.from(filenameToIdMap.entries()));
      ...
      // ✅ Map uploaded documents and match with /my documents to get correct IDs
      const uploadedMapped: Document[] = (uploadedResponse?.uploaded_documents || []).map((doc: any, idx: number) => {
        ...
        // ✅ Try to find matching document ID from /my endpoint
        ...
        console.log(`[Documents] 📄 Mapping document: ${doc.document_type}`, {
        ...
        return {
          ...
          uploadedId: documentId,  // ✅ ID from /my endpoint
      ...
      console.log("[Documents] 📊 Final mapped documents:", allDocs);
  ...
  console.log("[Documents] 🔍 VIEW DOCUMENT CALLED:");
  console.log("  Document object:", doc);
  console.log("  uploadedId:", doc.uploadedId);
  
  if (!doc.uploadedId) {
    console.error("[Documents] ❌ No document ID available");
    alert("Document ID not available. Cannot view document.");
    return;
  }

  const apiUrl = `/api/v1/documents/${doc.uploadedId}/view-url`;
  console.log(`[Documents] 📡 Making request to: ${apiUrl}`);

  try {
    setViewerLoading(true);
    setViewerModal(true); // ✅ Open modal immediately
    
    const response = await makeAuthenticatedRequest(apiUrl, { method: 'GET' });
    console.log('[Documents] ✅ View URL response:', response);

    const viewUrl = response.view_url || response.url || response.data?.view_url;

    if (viewUrl) {
      console.log('[Documents] ✅ Setting viewer URL:', viewUrl);
      setViewerUrl(viewUrl);
    } else {
      console.error('[Documents] ❌ No view_url in response:', response);
      setViewerModal(false);
      alert("Unable to generate view URL. Please try again.");
    }
  } catch (error: any) {
    console.error('[Documents] ❌ Failed to get view URL:', error);
    setViewerModal(false);
 ...
 const handleUploadOne = async (doc: Document) => {
  ...
  try {
    ...
    // ✅ NOW fetch /api/v1/documents/my to get the actual document ID
    console.log("[Documents] 🔍 Fetching all documents to find uploaded document ID...");
    ...
    // ✅ Find the document we just uploaded by matching filename
    ...
    if (!uploadedDoc) {
      console.error("[Documents] ❌ Could not find uploaded document in /my response");
    ...
    // ✅ Extract the document ID from /my response
    const documentId = uploadedDoc.id;
    console.log("[Documents] ✅ Found document ID for viewing:", documentId);
    ...
    // ✅ Update document with the correct ID for viewing
    setDocuments(prev =>
      prev.map(d =>
        d.id === doc.id
          ? {
              ...d,
              status: "uploaded",
              uploadedId: documentId,  // ✅ This is the ID from /my endpoint
              fileName: file.name,
              uploadDate: new Date().toLocaleDateString(),
              rejectionReason: undefined,
            }
...
{s3HealthChecked && s3Health === true && (
  ...
  onView={handleViewDocument}  // âœ… ADD THIS
...
<DocumentCard
  ...
  onFileSelect={handleFileSelect}
  onUploadOne={handleUploadOne}
  onView={handleViewDocument}   // ✅ FIX ADDED
  onRemoveFile={handleRemoveFile}
  s3Health={s3Health}
/>
