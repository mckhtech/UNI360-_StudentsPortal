                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <Badge 
                          className={`ml-2 text-xs ${
                            course.degree_type === 'phd' ? 'bg-purple-100 text-purple-800' :
                            course.degree_type === 'masters' ? 'bg-blue-100 text-blue-800' :
...
export default function Universities() {
  ...
  useEffect(() => {
    loadData();
  }, [selectedCountry, searchQuery, selectedCity, selectedState, selectedCourse, selectedLanguage, selectedDegreeType, selectedIntakeSeason, tuitionRange, gpaRange]);
  ...
  const loadData = async () => {
    try {
      ...
      if (selectedCountry === "DE") {
        ...
        universityParams.country = "United Kingdom";
      }
      ...
      let universitiesData = await universityAPI.getUniversities(universityParams);
      ...
      if (searchQuery.trim() && coursesArray.length > 0) {
        ...
        const courseUniversityIds = [...new Set(coursesArray.map(course => course.university))];
        ...
        for (const universityId of courseUniversityIds) {
          if (!finalUniversities.some(uni => uni.id === universityId)) {
            try {
              const additionalUni = await universityAPI.getUniversityById(universityId);
              if (additionalUni) {
      ...
      if (selectedDegreeType !== 'all' || selectedIntakeSeason !== 'all' || tuitionRange !== 'all' || gpaRange !== 'all') {
        const universityIdsWithMatchingCourses = [...new Set(coursesArray.map(course => course.university))];
        finalUniversities = finalUniversities.filter(uni => universityIdsWithMatchingCourses.includes(uni.id));
      }
  ...
  const calculateUniversityStats = (universitiesData, coursesData) => {
    ...
    universitiesData.forEach(university => {
      const universityCourses = coursesData.filter(course => course.university === university.id);
      ...
      stats[university.id] = {
        ...
        degreeTypes: [...new Set(universityCourses.map(course => course.degree_type))].filter(Boolean),
        subjects: [...new Set(universityCourses.map(course => course.subject_area))].filter(Boolean).slice(0, 3),
        languages: [...new Set(universityCourses.map(course => course.language))].filter(Boolean),
        tuitionRange: tuitionFees.length > 0 ? {
          min: Math.min(...tuitionFees),
          max: Math.max(...tuitionFees)
        } : null,
        intakeSeasons: [...new Set(universityCourses.map(course => course.intake_season))].filter(Boolean)
      };
    });
  ...
  const filteredUniversities = useMemo(() => {
    ...
    switch (activeFilter) {
      case "Top 50":
        ...
        break;
      case "High Match":
        filtered = filtered.filter(uni => uni.match_score && uni.match_score >= 80);
        break;
      case "Low Tuition":
      ...
      case "High Acceptance":
        filtered = filtered.filter(uni => uni.acceptance_rate && uni.acceptance_rate >= 20);
        break;
      default:
        break;
    }
          ...
          <div className="flex justify-between items-start mb-3">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              <div className="w-10 h-10 rounded-lg overflow-hidden shadow-sm flex-shrink-0">
                ...
                <div className={`w-full h-full bg-gradient-to-r from-[#C4DFF0] to-[#E08D3C] flex items-center justify-center ${university.image_url ? 'hidden' : ''}`}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[#2C3539] flex items-center mt-1">
                  ...
                  <span className="truncate">{university.city}{university.state ? `, ${university.state}` : ''}</span>
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-6 gap-3 mb-4">
            ...
            <div className="flex flex-col items-start justify-between">
              <span className="text-sm text-[#2C3539] font-medium whitespace-nowrap">
                {loading ? 'Loading...' : `${filteredUniversities.length} universities • ${courses.length} courses`}
              </span>
